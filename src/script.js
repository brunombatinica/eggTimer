document.addEventListener('DOMContentLoaded', function() {
    ...
    let placeHolderTime = 20*60; // variable to store the placeholder time
    ...
    function startTimer() {
        if (isTimerRunning) {  //if the timer is already running no need to read time left in seconds just "create the interval" - restart the timer
            ...
            timeLeftInSeconds = parseInt(timeInput.value)*60 || placeHolderTime; //parse the input value as an integer, if it's not a number, set it to 20
            //update the display
        ...
        eggTimerIntervalID = setInterval(() => {  //setInterval is a built-in javascript function that takes a callback function and a delay in milliseconds
            ...
            if (timeLeftInSeconds <= 0) {
                ...
                clearInterval(eggTimerIntervalID);
                timerDisplay.textContent = "Time's up!";
                beep();
                setTimeout(() => { //slight delay to allow the beep to play
                    alert("Time's up!");
                }, 500);
            } else {
                //timer is not running, stop the interval
                updateDisplay(timeLeftInSeconds);
            }
        }, 1000); // 1000ms =  1s
    }
    ...
    resetButton.addEventListener('click', () => {
        ...
        timeLeftInSeconds = parseInt(timeInput.value)*60 || placeHolderTime; //reset the time left in seconds
        updateDisplay(timeLeftInSeconds); //update the display
